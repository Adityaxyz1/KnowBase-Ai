...
export async function streamChat({
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.error('Chat API error:', resp.status, errorData);
...
export function analyzeConfidence(content: string): 'high' | 'medium' | 'low' {
  ...
  for (const phrase of lowConfidencePhrases) {
    if (lowerContent.includes(phrase)) return 'low';
  }
  
  // Count uncertain phrases
  let uncertainCount = 0;
  for (const phrase of uncertainPhrases) {
    if (lowerContent.includes(phrase)) uncertainCount++;
  }
  
  if (uncertainCount >= 3) return 'low';
